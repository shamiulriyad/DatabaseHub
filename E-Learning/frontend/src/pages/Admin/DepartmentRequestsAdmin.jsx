...
const DepartmentRequestsAdmin = () => {
  ...
  return (
    <Box p={6}>
      <Heading size="lg" mb={4}>Department Requests</Heading>
      {loading ? <Spinner /> : (
        <Table variant="simple">
          ...
          <Tbody>
            {requests.map(r => (
              <Tr key={r.id ?? r.Id}>
                <Td>{r.id ?? r.Id}</Td>
                <Td>{r.universityName ?? r.UniversityName ?? r.universityId ?? r.UniversityId}</Td>
                <Td>{r.departmentName ?? r.DepartmentName}</Td>
                <Td>{r.shortCode ?? r.ShortCode}</Td>
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Reject Department Request</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
