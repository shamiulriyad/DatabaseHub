...
import { FiCheckCircle, FiXCircle, FiClock, FiLayers, FiUser, FiCalendar, FiHash, FiAlertTriangle, FiGlobe } from 'react-icons/fi';
...
const getUni  = (r) => r.universityName ?? r.UniversityName ?? r.universityId ?? r.UniversityId ?? '—';
const getDept = (r) => r.departmentName ?? r.DepartmentName ?? '—';
const getCode = (r) => r.shortCode ?? r.ShortCode ?? '—';
...
const SkeletonRow = () => (
  <tr>
    {[6, 22, 20, 12, 18, 20, 16].map((w, i) => (
      <td key={i} style={{ padding: '16px 18px' }}>
        <div style={{
          height: 11, width: `${w}%`, minWidth: 28, borderRadius: 6,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <textarea
            ...
            style={{
              ...
              padding: '11px 14px', outline: 'none', resize: 'vertical', lineHeight: 1.6,
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} disabled={loading} style={{
            ...
            fontSize: 14, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.5 : 1,
          }}>Cancel</button>
    ...
    <td colSpan={7} style={{ padding: '72px 24px', textAlign: 'center' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        <div style={{
          ...
          background: t.infoSoft, border: `1px solid ${t.info}30`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}><FiLayers size={26} color={t.info} /></div>
        <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: t.textPrimary }}>
          No Pending Requests
        </p>
        <p style={{ fontSize: 13, color: t.textMuted }}>All department requests have been reviewed.</p>
      </div>
    </td>
  </tr>
);
...
const DepartmentRequestsAdmin = () => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const res  = await api.get('/admin/department-requests');
  ...
  const handleApprove = async (id) => {
    setActionLoading(id);
    try {
      await api.post(`/admin/department-requests/${id}/approve`);
      showToast('Department request approved!', 'success');
  ...
  const handleReject = async () => {
    ...
    setActionLoading(id);
    try {
      await api.post(`/admin/department-requests/${id}/reject`, { note });
  ...
  const cols = ['ID', 'University', 'Department', 'Short Code', 'Requested By', 'Created', 'Actions'];
  const colIcons = [FiHash, FiGlobe, FiLayers, null, FiUser, FiCalendar, null];
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        ...
        <div style={{
          ...
          background: `radial-gradient(ellipse 65% 45% at 10% 5%, rgba(139,92,246,0.08) 0%, transparent 55%),
                       radial-gradient(ellipse 50% 35% at 90% 90%, rgba(59,130,246,0.05) 0%, transparent 55%)`,
        }} />
      </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="fade-up" style={{ marginBottom: 32 }}>
            ...
            <h1 style={{
              ...
              marginBottom: 6,
            }}>Department Requests</h1>
            <p style={{ fontSize: 14, color: t.textSecondary }}>
              Review and approve or reject department creation requests.
            </p>
          </div>
          ...
          <div className="fade-up d3" style={{
            ...
            <div style={{
              ...
              <button
                ...
                onMouseEnter={e => { if (!loading) e.currentTarget.style.background = t.accent + '25'; }}
                              ...
                              <td style={{ padding: '15px 18px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{
                                    ...
                                    <FiGlobe size={13} color={t.accent} />
                                  </div>
                                  <span style={{ fontSize: 12, color: t.textSecondary }}>{getUni(r)}</span>
                                </div>
                              </td>
                              ...
                              <td style={{ padding: '15px 18px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{
                                    ...
                                    background: t.infoSoft, border: `1px solid ${t.info}25`,
                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                  }}>
                                    <FiLayers size={14} color={t.info} />
                                  </div>
                                  <span style={{ fontWeight: 600, fontSize: 13, color: t.textPrimary }}>{getDept(r)}</span>
                                </div>
                              </td>
                              ...
                              <td style={{ padding: '15px 18px' }}>
                                {getCode(r) !== '—' ? (
                                  <span style={{
                                    ...
                                    background: t.goldSoft, border: `1px solid ${t.gold}30`,
...
export default DepartmentRequestsAdmin;